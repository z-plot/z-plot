...
c = postscript(title='dgraid.eps', dimensions=[400,290])
p = plotter()
...
# a few labels
c.text(coord=nocopy.map([2250,8300]), text='DGRAID: Measuring Imperfect Placement')
c.text(coord=[13,150], text='Number of Misplaced Blocks (Thousands)', rotate=90)
 
# boxes around the closeup regions (though not really needed)
c.box(coord=copy.map([[650,-400],[770,6000]]), linedash=[2,2], linewidth=0.25, linecolor='black')
c.box(coord=copy.map([[3200,-400],[3400,1000]]), linedash=[2,2], linewidth=0.25, linecolor='black')
